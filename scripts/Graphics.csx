// Changes made by sioonhho - 9/13/2025
// Based off of ImportGraphicsAdvanced.csx
// Modified collision mask code to support all sprite frames and frames spread across separate atlasses
// Added support for palettes: credit to @PamTheDev
// Removed all GUI related code and prompts for use in a non-interactive program
...
static bool importFrameless = true;
...
static readonly List<string> ImageExtensions = [".png", ".gif"];

public void ImportSprites(Manifest mod) {
    string importFolder = mod.Directory.FullName;

    var hasImages = mod.Directory.EnumerateFiles("*.*", SearchOption.AllDirectories)
        .Any(file => ImageExtensions.Contains(file.Extension));

    if (!hasImages) {
        return;
    }

    logger.Debug(mod.Name, $"Checking sprite validity ...");


    CheckValidity(mod.Name, importFolder);

    string packDir = Path.Combine(ModsPath, "QuickLoader", "packager", mod.Directory.Name);
    ...
    try
    {
        ...
        logger.Info(mod.Name, $"Generating spritesheets ...");
        packer.Process(sourcePath, textureSize, PaddingValue, debug);
        packer.SaveAtlasses(outName);

        logger.Debug(mod.Name, $"Logs and spritesheets have been saved to {packDir}");

        int lastTextPage = Data.EmbeddedTextures.Count - 1;
        int lastTextPageItem = Data.TexturePageItems.Count - 1;

        bool bboxMasks = Data.IsVersionAtLeast(2024, 6);
        Dictionary<UndertaleSprite, Dictionary<int, (int, Node)>> maskNodesLookup = new();

        var numberOfNodes = packer.Atlasses
            .Select(atlas => atlas.Nodes.Where(node => node.Texture is not null).Count())
            .Sum();

        logger.Info(mod.Name, $"Importing {numberOfNodes} {(numberOfNodes > 1 ? "sprites" : "sprite")} ...");

        // Import everything into UTMT
        string prefix = outName.Replace(Path.GetExtension(outName), "");

        List<MagickImage> atlasImages = packer.Atlasses
            .Select((atlas, index) => {
                var image = TextureWorker.ReadBGRAImageFromFile(Path.Combine(packDir, $"{prefix}{index:000}.png"));
                imagesToCleanup.Add(image);

                return image;
            })
            .ToList();

        List<IPixelCollection<byte>> atlasPixelsList = atlasImages.Select(image => image.GetPixels()).ToList();

        for (var atlasIndex = 0; atlasIndex < packer.Atlasses.Count; atlasIndex++)
        {
            UndertaleEmbeddedTexture texture = new();
            texture.Name = new UndertaleString($"Texture {++lastTextPage}");
            texture.TextureData.Image = GMImage.FromMagickImage(atlasImages[atlasIndex]).ConvertToPng(); // TODO: other formats?
            Data.EmbeddedTextures.Add(texture);
            foreach (Node n in packer.Atlasses[atlasIndex].Nodes)
            {
                if (n.Texture != null)
                {
                    ...
                    else if (spriteType == SpriteType.Sprite)
                    {
                        ...
                        if (sprite is null)
                        {
                            ...
                            if (isHurtbox) {
                                maskNodesLookup.Add(newSprite, new() { { frame, (atlasIndex, n) } });
                            }
                        ...
                        if (isHurtbox && !maskNodesLookup.Any(node => node.Key.Name == sprite.Name)) {
                            maskNodesLookup.Add(sprite, new());
                        }
                        ...
                        if (isHurtbox && !maskNodesLookup[sprite].ContainsKey(frame)) {
                            maskNodesLookup[sprite].Add(frame, (atlasIndex, n));
                        }
            ...
            foreach (var (maskSpr, maskNodes) in maskNodesLookup)
            {
                maskSpr.CollisionMasks.Clear();

                foreach (var (frame, (atlas, maskNode)) in maskNodes.OrderBy(x => x.Key))
                {
                    // Generate collision mask using either bounding box or sprite dimensions
                    var maskEntry = maskSpr.NewMaskEntry(Data);
                    ...
                    for (int y = 0; y < maskHeight && y < maskNode.Bounds.Height; y++)
                    {
                        ...
                        {
                            IMagickColor<byte> pixelColor = atlasPixelsList[atlas].GetPixel(x + maskNode.Bounds.X, y + maskNode.Bounds.Y).ToColor();
                            if (bboxMasks)
                            {
                    ...
                    for (int i = 0; i < bytes.Length; i++)
                        maskEntry.Data[i] = bytes[i];

                    maskSpr.CollisionMasks.Add(maskEntry);
                }

                maskNodes.Clear();
            }

            maskNodesLookup.Clear();
        }
    }
    finally
    {
...
void CheckValidity(string modName, string importFolder)
{
